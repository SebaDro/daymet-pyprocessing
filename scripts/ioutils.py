...
def discover_yearly_daymet_files_for_id_and_variable(root_data_dir: str, id: str, variables: list, version: str):
    ...
    for variable in variables:
        ...
        if version == "v3":
            pattern = f"{data_dir}/{id}_daymet_v3_{variable}_*_na.nc4"
        elif version == "v4":
            pattern = f"{data_dir}/{id}_daymet_v4_daily_na_{variable}_*.nc"
        else:
            pattern = f"{data_dir}/{id}_daymet_v4_daily_na_{variable}_*.nc"
        files = glob.glob(pattern)
        if not files:
...
def discover_daymet_file_for_id(data_dir: str, basin_id: str, version: str):
    ...
    {root_dir}/{id}_daymet_v3_na.nc4. These patterns follow the naming style for storing  results of the 'combine()
    process.
